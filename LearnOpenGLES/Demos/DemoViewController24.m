 ...
 *
 * 你可以检测w分量是否等于1.0，来检测它是否是光的位置向量；w分量等于0.0，则它是光的方向向量。
 */
struct _Light
{
...
@interface DemoViewController24 () <GLKViewDelegate>
{
...
@implementation DemoViewController24
...
- (void)viewDidLoad
{
    ...
    // shader
    self.lightProgram = [[GLProgram alloc] initWithVertexShaderFilename:@"shaderv_24_0" fragmentShaderFilename:@"shaderf_24_0"];
    [self.lightProgram link];
    
    self.modelProgram = [[GLProgram alloc] initWithVertexShaderFilename:@"shaderv_24_1" fragmentShaderFilename:@"shaderf_24_1"];
...
//        glBindBuffer(GL_UNIFORM_BUFFER, 0);
    ...
    // 灯光
    _light.direction = GLKVector4Make(0.0, -1.0, 1.0, 1.0);
    _lightMatrix = GLKMatrix4Multiply(GLKMatrix4MakeTranslation(-_light.direction.x, -_light.direction.y, -_light.direction.z), GLKMatrix4MakeScale(0.25, 0.25, 0.25));

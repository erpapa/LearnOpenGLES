...
- (void)initializeMetalLayer
{
    ...
    metalLayer.drawableSize = CGSizeMake(self.layer.bounds.size.width * self.layer.contentsScale, self.layer.bounds.size.height * self.layer.contentsScale);
#endif
}
...
- (void)initializeFilament
{
    ...
    filaView->setViewport(Viewport(0, 0, self.layer.bounds.size.width * self.layer.contentsScale, self.layer.bounds.size.height * self.layer.contentsScale));
}

...
void main(void) {
   ...
   vec3 vcoeff = vec3(0.439271, -0.367833, -0.071438);
    ...
    if (nowTxtPos.y < 0.25) {
        ...
        basePos.y -= 2.0;
        vec2 samplingPos = basePos / size;
        vec4 texel = texture2D(aTexture0, samplingPos).rgba;
        float u1 = dot(texel.rgb, vcoeff);
        u1 += offset.y;
        basePos.x += 2.0;
         samplingPos = basePos / size;
        texel = texture2D(aTexture0, samplingPos).rgba;
        float u2 = dot(texel.rgb, vcoeff);
        u2 += offset.y;
        basePos.x += 2.0;
        samplingPos = basePos / size;
        texel = texture2D(aTexture0,samplingPos).rgba;
        float u3 = dot(texel.rgb, vcoeff);
        u3 += offset.y;
        basePos.x += 2.0;
        samplingPos = basePos / size;
        texel = texture2D(aTexture0, samplingPos).rgba;
        ...
        vec2 samplingPos = basePos / size;
        vec4 texel = texture2D(aTexture0, samplingPos).rgba;
        float v1 = dot(texel.rgb, ucoeff);
        v1 += offset.z;
        basePos.x += 2.0;
        samplingPos = basePos / size;
        texel = texture2D(aTexture0, samplingPos).rgba;
        float v2 = dot(texel.rgb, ucoeff);
        v2 += offset.z;
        basePos.x += 2.0;
        samplingPos = basePos / size;
        texel = texture2D(aTexture0, samplingPos).rgba;
        float v3 = dot(texel.rgb, ucoeff);
        v3 += offset.z;
        basePos.x += 2.0;
        samplingPos = basePos / size;
        texel = texture2D(aTexture0, samplingPos).rgba;
        float v4 = dot(texel.rgb, ucoeff);
        v4 += offset.z;
